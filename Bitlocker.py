# Name: Syed Fassih ul Hassny, Saad Umar, Hunain Raza, Ahmed Umar Rehman
# Roll Number: i221730, i221679, i221614, i221580
# Section: CY-A
# DF Project: Bitlocker Key Finder & Extractor


import re  # For regex to extract keys
...
# subprocess.run([r'install_dependencies.bat'], check=True)
        ...
        for para in doc.paragraphs:
            ...
            if key_match:
                for key in key_match:
                    ...
                    data.append(
                        {"File Path": file, "Recovery Key ID": recovery_id, "BitLocker Key": key})
                    gui.log_message(
                        f"File {file} Key ID {recovery_id} Bitlocker Key {key}", "info")
...
def parse_rtf(file):
    try:
        with open(file, 'rb') as f:  # Open in binary mode
            rtf_content = f.read()
            # Decode to proper encoding
            text = rtf_to_text(rtf_content.decode('utf-8'))
...
def name_search(gui):
    for ele in txt_Files:
        if fnmatch.fnmatch(ele, "*BitLocker Recovery Key*"):
            Bit_Keys.append(ele)
            gui.log_message(
                f"Found BitLocker Recovery Key file: {ele}", "success")
            ...
            try:
                ...
                if key_match:
                    for key in key_match:
                        recovery_id = id_match[0] if id_match else "Unknown"
                        data.append(
                            {"File Path": ele, "Recovery Key ID": recovery_id, "BitLocker Key": key})
                for key in key_match:
                    Bit_Keys.append(ele)
                    gui.log_message(
                        f"Found BitLocker key in file: {ele}", "success")
                    gui.log_message(f"Key: {key}", "info")
                    Collected_Keys.append(key)
            except UnicodeDecodeError:
                gui.log_message(
                    f"Unable to decode file as {encoding}: {ele}", "warning")
...
def make_key_report(report_folder):
    ...
    csv_file = os.path.join(
        output_folder, "BitlockerKeyReport" + now.strftime("%Y%m%d%H%M%S") + ".csv")
    with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(
            file, fieldnames=["File Path", "Recovery Key ID", "BitLocker Key"])
        writer.writeheader()
...
class BitlockerKeyFinderGUI:
    def __init__(self, master):
        self.master = master
        master.title("Bitlocker Key Finder and Extractor")
    ...
    def handle_memory_dump(self):
        """
        Handle the memory dump processing logic in a separate thread to keep the GUI responsive.
        """

        file_path = filedialog.askopenfilename(
            title="Select Memory Dump File",
            filetypes=(("Memory Dump Files", "*.mem *.dd *.raw"),
                       ("All Files", "*.*")),
        )
        if not file_path:
            self.log_message("No memory dump file selected.", "warning")
            return

        self.log_message(f"Processing memory dump file: {file_path}", "info")

        # Run the processing in a separate thread to avoid freezing the GUI
        thread = threading.Thread(
            target=self.process_memory_dump, args=(file_path,))
        thread.start()

    def process_memory_dump(self, file_path):
        """
        Extract potential BitLocker recovery keys from a memory dump file.
        Run in a separate thread to avoid blocking the GUI.
        """
        try:
            # Command to extract keys using strings and grep
            print("Processing")

            # Path to Git Bash executable
            # Adjust this if Git Bash is installed elsewhere
            git_bash_path = r"C:\Program Files\Git\bin\bash.exe"

            # Command to execute
            command = f"strings {
                file_path} | grep -E '[0-9]{{6}}-[0-9]{{6}}-[0-9]{{6}}-[0-9]{{6}}-[0-9]{{6}}-[0-9]{{6}}-[0-9]{{6}}-[0-9]{{6}}'"

            # Run the command in Git Bash
            result = subprocess.run(
                [git_bash_path, '-c', command], capture_output=True, text=True, check=True
            )

            keys = result.stdout.strip()

            if keys:
                self.log_message(f"BitLocker keys found:\n{keys}", "success")
            else:
                self.log_message(
                    "No BitLocker keys found in the memory dump.", "warning"
                )
        except subprocess.CalledProcessError as e:
            self.log_message(
                f"Error processing memory dump: {str(e)}", "error")

    def capture_live_ram(self):
        """
        Capture live RAM using winpmem_mini_x64_rc2.exe and save it to the specified location.
        """
        def run_capture():
            # Ask the user for save location
            save_path = filedialog.askdirectory(
                title="Select Folder to Save RAM Dump")  # Ask user for save location
            if not save_path:
                self.log_message("No folder selected for RAM dump.", "warning")
                return

            # Define the file name and full path for the dump
            dump_file = os.path.join(save_path, "live_ram_dump.raw")

            # Path to the winpmem_mini_x64_rc2.exe
            # Update this with the correct path if needed
            winpmem_path = "winpmem_mini_x64_rc2.exe"
            command = [winpmem_path, dump_file]

            try:
                # Run the winpmem command to capture live RAM and capture both stdout and stderr
                self.log_message(
                    f"Starting live RAM capture. Saving to: {dump_file}", "info")
                ...
                if result.returncode == 0:
                    self.log_message(
                        f"Live RAM dump saved successfully to: {dump_file}", "success")
                else:
                    # Log the error output if the command failed
                    self.log_message(
                        f"Error capturing live RAM. Exit code: {result.returncode}", "error")
                    self.log_message(f"stderr: {result.stderr}", "error")
                    self.log_message(f"stdout: {result.stdout}", "info")

            except subprocess.CalledProcessError as e:
                self.log_message(
                    f"Error capturing live RAM: {str(e)}", "error")
            except Exception as e:
    ...
    def run_efdd(self):
        """
        Run the efdd.exe program located at the specified path.
        """
        efdd_path = r"Elcomsoft Password Recovery\Elcomsoft Forensic Disk Decryptor\efdd.exe"
        try:
            self.log_message(
                f"Attempting to run efdd.exe from: {efdd_path}", "info")
            subprocess.run([efdd_path], check=True)
            self.log_message("efdd.exe executed successfully.", "success")
        except FileNotFoundError:
            self.log_message(
                f"efdd.exe not found at {efdd_path}. Please verify the path.", "error")
        except subprocess.CalledProcessError as e:
            self.log_message(
                f"Error executing efdd.exe: {str(e)}", "error")
        except Exception as e:
            self.log_message(f"Unexpected error: {str(e)}", "error")

    def browse_source(self):
        """
        Handle the browsing of the source directory.
        """
    ...
    def run_HxD(self):
        try:
            ...
            subprocess.run([r'C:\Program Files\HxD\HxD.exe'], check=True)
            self.log_message("HxD executed successfully.", "success")
        except Exception as e:
            self.log_message(f"Failed to run HxD: {e}", "error")

    def create_widgets(self):

        # Find Saved Bitlocker .TXT and .BEK Files
        tk.Label(self.master, text="Find Saved Bitlocker .TXT and .BEK Files", font=(
            "Arial", 12, "bold"), bg="#f0f0f0").pack(pady=5)

        # Source Directory
        source_frame = tk.Frame(self.master, bg="#f0f0f0")
        source_frame.pack(fill="x", padx=10, pady=5)
        tk.Label(source_frame, text="Select Volume or Directory to Search:",
                 bg="#f0f0f0").pack(side="left")
        self.source_entry = tk.Entry(source_frame, width=50)
        self.source_entry.pack(side="left", expand=True, fill="x", padx=5)
        tk.Button(source_frame, text="Browse", command=self.browse_source,
                  bg="#4CAF50", fg="white", relief=tk.RAISED).pack(side="left")

        # Checkboxes for search options
        checkbox_frame = tk.Frame(self.master, bg="#f0f0f0")
        checkbox_frame.pack(pady=5)
        self.filename_var = tk.BooleanVar()
        tk.Checkbutton(checkbox_frame, text="File Name Search (Fast)",
                       variable=self.filename_var, bg="#f0f0f0").pack(side="left")

        self.utf16le_search_var = tk.BooleanVar()
        tk.Checkbutton(checkbox_frame, text="UTF-16LE String Search",
                       variable=self.utf16le_search_var, bg="#f0f0f0").pack(side="left")

        self.exhaustive_search_var = tk.BooleanVar()
        tk.Checkbutton(checkbox_frame, text="Exhaustive Search - All txt, docx, xlsx, & rtf files (Slow)",
                       variable=self.exhaustive_search_var, bg="#f0f0f0").pack(side="left")

        # Copy Files Option
        self.copy_var = tk.BooleanVar()
        tk.Checkbutton(self.master, text="Copy responsive files to Output Directory",
                       variable=self.copy_var, bg="#f0f0f0").pack(pady=5)

        # Recover Keys from Current Machine
        tk.Label(self.master, text="Recover Keys from Current Machine",
                 font=("Arial", 12, "bold"), bg="#f0f0f0").pack(pady=5)
        self.mgbde_var = tk.BooleanVar()
        tk.Checkbutton(self.master, text="ADMIN ONLY - Save keys for mounted volumes to Output Directory",
                       variable=self.mgbde_var, bg="#f0f0f0").pack()

        # Output Directory
        output_frame = tk.Frame(self.master, bg="#f0f0f0")
        output_frame.pack(fill="x", padx=10, pady=5)
        tk.Label(output_frame, text="Output Directory:", font=(
            "Arial", 11, "bold"), bg="#f0f0f0").pack(side="left")
        ...
        self.find_keys_button = tk.Button(
            button_frame, text="Find Keys", command=self.start_find_keys_thread, bg="#2196F3", fg="white", relief=tk.RAISED)
        self.find_keys_button.pack(side="left", padx=5)
        self.process_memory_dump_button = tk.Button(
            button_frame, text="Process Memory Dump", command=self.handle_memory_dump, bg="#2196F3", fg="white", relief=tk.RAISED)
        self.process_memory_dump_button.pack(side="left", padx=5)
        tk.Button(button_frame, text="Help", command=self.show_help,
                  bg="orange", fg="black", relief=tk.RAISED).pack(side="left", padx=5)
        # Create a button widget
        run_HxD_button = tk.Button(
            self.master,
            text="Run HxD",
            command=self.run_HxD,
            bg="#4CAF50",
            fg="white",
            relief=tk.RAISED
        )
        run_HxD_button.pack(pady=10)

        # Button for capturing live RAM
        self.capture_button = tk.Button(self.master, text="Capture Live RAM",
                                        command=self.capture_live_ram, bg="#4CAF50", fg="white", relief=tk.RAISED)
        self.capture_button.pack(pady=10)

        # Add "Get Volume Master Key" button
        self.get_volume_key_button = tk.Button(
            self.master,
            text="Get Volume Master Key",
            command=self.run_efdd,
            bg="#4CAF50",
            fg="white",
            font=("Arial", 12, "bold")
        )
        self.get_volume_key_button.pack(pady=10)
        ...
        self.progress = ttk.Progressbar(
            self.master, orient="horizontal", length=800, mode="determinate")
        self.progress.pack(pady=0)
        clear_frame = tk.Frame(self.master, bg="#f0f0f0")
        clear_frame.pack(fill="x", padx=10, pady=(0, 10)
                         )  # Add padding at the bottom
        tk.Button(clear_frame, text="Clear Window", command=self.clear_console,
                  bg="#FF5722", fg="white", relief=tk.RAISED).pack()
    ...
    def find_keys(self):
        ...
        if self.filename_var.get():
            self.log_message(
                f"Searching for file names in {folder_path}", "info")
            name_search(self)

        if self.exhaustive_search_var.get():
            self.log_message(
                f"Conducting exhaustive string search in {folder_path}", "info")
        ...
        if self.utf16le_search_var.get():
            self.log_message(
                f"Conducting UTF-16LE string search in {folder_path}", "info")
            UTF16LE_string_search(self)

        self.log_message(
            f"Total BitLocker keys/files found: {len(Bit_Keys)}", "success")
    ...
    def copy_key_files(self):
        ...
        if not os.path.isdir(output_folder):
            self.log_message(
                "Invalid output directory. Please select a valid directory.", "warning")
            return
        for file in Bit_Keys:
            try:
                ...
                self.log_message(
                    f"Error copying file {file}: {str(e)}", "error")

    def get_active_keys(self):
        if not isAdmin():
            self.log_message(
                "Admin rights are required to retrieve BitLocker keys.", "warning")
            return
        output_folder = self.output_entry.get()
        # gets target computer name for report title
        comp_name = os.environ['COMPUTERNAME']
        comp_name = comp_name.strip('\\')
        key_report = os.path.join(
            output_folder, comp_name + '-BitlockerReport.txt')
        Drive_letters = ['%s:' % d for d in string.ascii_uppercase if os.path.exists(
            '%s:' % d)]  # Produces list of volumes on target system
        ...
        try:
            ...
            volumes = subprocess.check_output(
                ["manage-bde", "-status"]).decode("utf-8")
            ...
            with open(key_report, "a") as key_file:
                for line in volume_lines:
                    ...
                    if "Volume " in line:
                        ...
                        print(volume)
                        try:
                            recovery_keys = subprocess.check_output(
                                ["manage-bde", "-protectors", "-get", volume], startupinfo=startupinfo).decode("utf-8")
                            key_file.write(
                                f"Bitlocker key found for {volume}!\n\n")
                            key_file.write(recovery_keys)
                            self.log_message(
                                f"BitLocker key for volume {volume} written to report at {key_report}", "success")
                            # self.log_message(f"{recovery_keys}", "info")
                        except subprocess.CalledProcessError:
                            # self.log_message(f"No BitLocker credentials found for {volume}", "warning")
                            key_file.write(
                                f"No BitLocker credentials found for {volume}\n\n")
        except Exception as e:
            self.log_message(
                f"Error retrieving BitLocker keys: {str(e)}", "error")
